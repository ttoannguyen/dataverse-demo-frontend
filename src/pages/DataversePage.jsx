  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        const countries = [
          ...new Set(
            fetchedDatasets.map((ds) => ds.metadata?.country).filter(Boolean)
          ),
        ];
        const languages = [
          ...new Set(
            fetchedDatasets.map((ds) => ds.metadata?.language).filter(Boolean)
          ),
        ];
        const formats = [
          ...new Set(
            fetchedDatasets.map((ds) => ds.metadata?.format).filter(Boolean)
          ),
        ];
        const organizations = [
          ...new Set(
        ...
        const topics = [
          ...new Set(
            fetchedDatasets.map((ds) => ds.metadata?.topic).filter(Boolean)
          ),
        ];
        const licenses = [
          ...new Set(
            fetchedDatasets.map((ds) => ds.metadata?.license).filter(Boolean)
          ),
        ];
